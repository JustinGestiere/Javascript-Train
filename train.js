...
function dessine_case(contexte, plateau, x, y){
	...
	// NOTE: à améliorer
	if (la_case.nom == 'rail horizontal'|| la_case.nom == 'rail vertical' || la_case.nom == 'rail droite vers haut' || la_case.nom == 'rail haut vers droite' || la_case.nom == 'rail droite vers bas' || la_case.nom == 'rail bas vers droite') {
		contexte.drawImage(IMAGE_GRIS, x * LARGEUR_CASE, y * HAUTEUR_CASE, LARGEUR_CASE, HAUTEUR_CASE);
	}
...
function tchou(){
	...
	const souris = document.getElementById("simulateur");
	souris.addEventListener(("mousemove","click"), (event) => {
		let rect = souris.getBoundingClientRect();
  		let mouseX = event.clientX - rect.left;
  		let mouseY = event.clientY - rect.top;
		
		
		let x = Math.trunc(mouseX / LARGEUR_CASE)
		let y = Math.trunc(mouseY / HAUTEUR_CASE)
		let positioncase = [x,y]
		
		const boutonselectionne = document.querySelector("button:disabled");
		const selectedbyid = boutonselectionne.id

		plateau.cases[x][y] = typecases(selectedbyid);
